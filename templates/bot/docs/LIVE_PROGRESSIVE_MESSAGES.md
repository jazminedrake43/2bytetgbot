...
By default, the progressive message uses the current classic layout with status icons.

#### `setStyle(style)`
Switches the renderer for the current instance.

```typescript
msgProgressive.setStyle("matrix");
```
...
| Style | Description |
|---|---|
| `default` | Current classic output with Telegram-friendly status icons |
| `matrix` | Tech-style output with system prefixes, node labels, and binary-like signals |

#### `matrixStyle()`
Convenience shorthand for `setStyle("matrix")`.
...
Use this when you want a more cinematic "terminal scan" look without changing the existing default style.
